...
test_jvm() {
  ...
  if [ "$ver_major" -lt "12" ]; then
    return;
  elif [ "$ver_major" -eq "12" ]; then
    if [ "$ver_minor" -lt "0" ]; then
      return;
    elif [ "$ver_minor" -eq "0" ]; then
      if [ "$ver_micro" -lt "2" ]; then
        return;
      fi
    fi
  fi
...
tail -c 81656288 "$prg_dir/${progname}" > sfx_archive.tar.gz 2> /dev/null
if [ "$?" -ne "0" ]; then
  tail -81656288c "$prg_dir/${progname}" > sfx_archive.tar.gz 2> /dev/null
  if [ "$?" -ne "0" ]; then
...
if [ -z "$app_java_home" ]; then
  ...
  echo The version of the JVM must be at least 12.0.2.
...
$INSTALL4J_JAVA_PREFIX "$app_java_home/bin/java" -Dexe4j.moduleName="$prg_dir/$progname" -Dexe4j.totalDataLength=189069064 -Dinstall4j.cwd="$old_pwd" "-Dsun.java2d.noddraw=true" "$vmov_1" "$vmov_2" "$vmov_3" "$vmov_4" "$vmov_5" $INSTALL4J_ADD_VM_PARAMS -classpath "$local_classpath" install4j.Installer3680162217  "$@"
return_code=$?
else
$INSTALL4J_JAVA_PREFIX "$app_java_home/bin/java" -Dexe4j.moduleName="$prg_dir/$progname" -Dexe4j.totalDataLength=189069064 -Dinstall4j.cwd="$old_pwd" "-Dsun.java2d.noddraw=true" $INSTALL4J_ADD_VM_PARAMS -classpath "$local_classpath" install4j.Installer3680162217  "$@"
return_code=$?
fi
